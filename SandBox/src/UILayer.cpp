...
UILayer::UILayer(Ref<Scene> scene) : Layer("UILayer"), m_ActiveScene(scene) {
...
void UILayer::OnUpdate(Runic2D::Timestep ts)
{
    ...
    Entity debugEntity = m_ActiveScene->FindEntityByName("DebugInfo");
    if (debugEntity)
    {
        auto& text = debugEntity.GetComponent<TextComponent>();
        text.TextString = "FPS: " + std::to_string((int)fps);
    }
